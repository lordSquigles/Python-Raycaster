...
def render(player, map, window, wallTex, constants): # my implementation of lodev's DDA algorithm ()
    for x in range(0, constants['screenW'], 1): # I will draw slices in quantity equal to 1/4 of the screenW but 4px wide
        ...
        if hit:
            ...
            #slice = [wallTex.array[int(textureX) + int((j * (wallTex.h / height))) * wallTex.w] for j in range(int(height))] # some list comprehension to make our textured slice in one line
            j = 0
            while j < height:
                #pygame.draw.rect(window, slice[j], [x, int(player.horizon - height / 2) + j, 4, 1])
                pygame.draw.rect(window, wallTex.array[int(textureX) + int((j * (wallTex.h / height))) * wallTex.w], [x, int(player.horizon - height / 2) + j, 1, 1])
                j += 1
